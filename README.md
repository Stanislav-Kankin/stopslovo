...
Дополнительно подключен офлайн-лексикон РАН `backend/app/data/ran_lexicon.json`, собранный из PDF-словарей через `backend/tools/build_ran_lexicon.py`. Он используется как вспомогательный нормативный слой после `pymorphy3`: если слово найдено в лексиконе РАН и не находится в риск-словаре, оно не отправляется в LLM.
...
```bash
cd backend
python tools/build_ran_lexicon.py "/path/to/Archive 3.zip"
```
