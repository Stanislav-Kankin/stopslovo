...
Дополнительно подключена офлайн-база нормативных словарей `backend/app/data/lexicon.sqlite`, собранная из PDF-словарей через `backend/tools/build_ran_lexicon.py`. Она используется как вспомогательный нормативный слой после `pymorphy3`: если слово найдено в базе словарей и не находится в риск-словаре, оно не отправляется в LLM.

Сейчас в базе 4 нормативных источника:
...
Пересобрать лексикон РАН из PDF/zip-архива:

```bash
cd backend
python tools/build_ran_lexicon.py "/path/to/Словари.zip"
```

Скрипт создает `backend/app/data/lexicon.sqlite` и совместимый JSON `backend/app/data/ran_lexicon.json`.
