...
RAN_SOURCE_NOTE = (
    "Сверка с нормативной базой: слово не найдено в подключённых орфографическом, "
    "орфоэпическом, словаре иностранных слов и толковом словаре государственного языка."
)
...
class DictionaryChecker:
        ...
        for match in latin_matches:
            ...
            if entry:
                ...
                found.setdefault((normalized, "latin"), {
                    ...
                    "replacements": [],
                    "sources": [
                        f"Словарь риска СтопСлово v{self.version}: слово не найдено в локальной базе исключений и освоенных терминов.",
                        RAN_SOURCE_NOTE,
                    ],
                    "keep_as_is": True,
    ...
    def _issue(self, token: dict, entry: dict, context_type: str) -> dict:
        ...
        return {
            ...
            "sources": [
                f"Словарь риска СтопСлово v{self.version}: термин включён в базу англицизмов и заимствований.",
                f"Категория методики: {entry.get('category', 'не указана')}.",
            ],
